...
export const useStakingData = (
  ...
  const myValidatorsQuery =
    cosmosQuery.staking.v1beta1.useDelegatorValidators(
      {
        request: {
    ...
    cosmosQuery.distribution.v1beta1.useDelegationTotalRewards(
      {
        ...
        options: {
          enabled: isDataQueryEnabled,
          select: (data) =>
            parseRewards(data, coin.base, -exp),
        },
      },
    );
  ...
  const validatorsQuery =
    cosmosQuery.staking.v1beta1.useValidators({
      request: {
        status:
          cosmos.staking.v1beta1.bondStatusToJSON(
  ...
  const unbondingDaysQuery =
    cosmosQuery.staking.v1beta1.useParams({
      options: {
        enabled: isDataQueryEnabled,
        select: ({ params }) =>
          parseUnbondingDays(params),
      },
    });

  const annualProvisionsQuery =
    cosmosQuery.mint.v1beta1.useAnnualProvisions({
      options: {
        enabled: isDataQueryEnabled,
  ...
  const poolQuery =
    cosmosQuery.staking.v1beta1.usePool({
      options: {
        enabled: isDataQueryEnabled,
        select: ({ pool }) => pool,
      },
  ...
  const communityTaxQuery =
    cosmosQuery.distribution.v1beta1.useParams({
      options: {
        enabled: isDataQueryEnabled,
        select: ({ params }) =>
          shiftDigits(
            params?.communityTax || '0',
            -18,
          ),
      },
    });
  ...
  useEffect(() => {
    queriesWithUnchangingKeys.forEach((query) =>
      query.remove(),
    );
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [chainName]);

  const isInitialFetching = Object.values(
    allQueries,
  ).some(({ isLoading }) => isLoading);

  const isRefetching = Object.values(
    allQueries,
  ...
  const data = useMemo(() => {
    ...
    const queriesData = Object.fromEntries(
      Object.entries(allQueries).map(
        ([key, query]) => [key, query.data],
      ),
    ) as QueriesData;
    ...
    const totalDelegated =
      calcTotalDelegation(delegations);
  ...
  const refetch = () => {
    updatableQueriesAfterMutation.forEach(
      (query) => query.refetch(),
    );
  };
