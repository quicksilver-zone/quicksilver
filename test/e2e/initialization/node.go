...
func (n *internalNode) buildCreateValidatorMsg(amount sdk.Coin) (sdk.Msg, error) {
	...
	addr, err := n.keyRecord.GetAddress()
	if err != nil {
		return nil, err
	}
...
func (n *internalNode) createAppConfig(nodeConfig *NodeConfig) {
	...
	appConfig.MinGasPrices = fmt.Sprintf("%s%s", MinGasPrice, QuickDenom)
...
func (n *internalNode) createKeyFromMnemonic(name, mnemonic string) error {
	...
	kr, err := keyring.New(keyringAppName, keyring.BackendTest, n.configDir(), inBuf, util.Cdc)
	if err != nil {
		return err
	}
	...
	info, err := kr.NewAccount(name, mnemonic, "", sdk.FullFundraiserPath, algo)
	if err != nil {
		return err
	}

	privKeyArmor, err := kr.ExportPrivKeyArmor(name, keyringPassphrase)
	if err != nil {
		return err
	}
...
func (n *internalNode) export() (*Node, error) {
	...
	pubkey, err := n.keyRecord.GetPubKey()
	if err != nil {
		return nil, err
	}
...
func (n *internalNode) init() error {
	...
	appState, err := json.MarshalIndent(quicksilver.ModuleBasics.DefaultGenesis(util.Cdc), "", " ")
	if err != nil {
