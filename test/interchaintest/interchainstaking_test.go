...
func TestInterchainStaking(t *testing.T) {
	...
	require.NoError(t, err)
	gaiaUser, err := interchaintest.GetAndFundTestUserWithMnemonic(ctx, t.Name(), mnemonic, genesisWalletAmount.Int64(), gaia)
	require.NoError(t, err)
	...
	performanceAddress := zone[0].PerformanceAddress
	queryICA = []string{
	...
	// Transfer ibc token from Gaia -> Quicksilver
	...
	transferTx, err := gaia.SendIBCTransfer(ctx, "channel-0", gaiaUserAddr, transfer, ibc.TransferOptions{})
	require.NoError(t, err)
