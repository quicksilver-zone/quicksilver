...
func TestGovSetLsmCaps(t *testing.T) {
	cases := []struct {
		...
		},
		{
			...
			Msg:  types.MsgGovSetLsmCaps{Title: "test", Description: "test", ChainId: "chain-1", Caps: &types.LsmCaps{ValidatorCap: sdkmath.LegacyOneDec(), ValidatorBondCap: sdkmath.LegacyZeroDec(), GlobalCap: sdkmath.LegacyNewDecWithPrec(50, 2)}, Authority: addressutils.GenerateAddressForTestWithPrefix("quick")},
			Err:  "validator bond cap must be greater than 0",
		},
		{
