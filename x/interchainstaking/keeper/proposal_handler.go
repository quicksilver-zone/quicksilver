...
func (k *Keeper) HandleUpdateZoneProposal(ctx sdk.Context, p *types.UpdateZoneProposal) error {
	...
	for _, change := range p.Changes {
		if err := k.applyChange(ctx, &zone, *change); err != nil {
			return err
		}
	}
...
func (k *Keeper) applyChange(ctx sdk.Context, zone *types.Zone, change types.UpdateZoneValue) error {
	switch change.Key {
	case "base_denom", "local_denom":
		if err := sdk.ValidateDenom(change.Value); err != nil {
			return err
		}
		if k.BankKeeper.GetSupply(ctx, zone.LocalDenom).Amount.IsPositive() {
			return errors.New("zone has assets minted, cannot update denom without potentially losing assets")
		}
	...
	case "liquidity_module", "unbonding_enabled", "deposits_enabled", "return_to_sender", "is_118":
	...
	case "connection_id":
		if err := k.applyConnectionIDChange(ctx, zone, change.Value); err != nil {
			return err
		}
...
func (k *Keeper) applyConnectionIDChange(ctx sdk.Context, zone *types.Zone, connectionID string) error {
	...
	return k.registerInterchainAccount(ctx, zone.ConnectionId, portOwner)
}
