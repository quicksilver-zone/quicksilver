...
func (k *Keeper) GetUnlockedTokensForZone(ctx sdk.Context, zone *types.Zone) (map[string]math.Int, math.Int, error) {
	...
	for _, redelegation := range k.ZoneRedelegationRecords(ctx, zone.ZoneID()) {
		...
		if found {
			...
			if availablePerValidator[redelegation.Destination].LT(sdk.ZeroInt()) {
				return map[string]math.Int{}, sdk.ZeroInt(), fmt.Errorf("negative available amount [chain: %s, validator: %s, amount: %s]; unable to continue", zone.ZoneID(), redelegation.Destination, availablePerValidator[redelegation.Destination].String())
			}
...
func (k *Keeper) HandleQueuedUnbondings(ctx sdk.Context, zone *types.Zone, epoch int64) error {
	...
	k.IterateZoneStatusWithdrawalRecords(ctx, zone.ZoneID(), types.WithdrawStatusQueued, func(idx int64, withdrawal types.WithdrawalRecord) bool {
...
WITHDRAWAL:
	...
	for _, hash := range utils.Keys(txDistrsMap) {
		record, found := k.GetWithdrawalRecord(ctx, zone.ZoneID(), hash, types.WithdrawStatusQueued)
		if !found {
	...
	for _, valoper := range utils.Keys(valOutCoinsMap) {
		if !valOutCoinsMap[valoper].Amount.IsZero() {
			...
			k.SetUnbondingRecord(ctx, types.UnbondingRecord{ChainId: zone.ZoneID(), EpochNumber: epoch, Validator: valoper, RelatedTxhash: txHashes[valoper]})
		}
	}
...
func (k *Keeper) GCCompletedUnbondings(ctx sdk.Context, zone *types.Zone) error {
	var err error
	k.IterateZoneStatusWithdrawalRecords(ctx, zone.ZoneID(), types.WithdrawStatusCompleted, func(idx int64, withdrawal types.WithdrawalRecord) bool {
