...
func (k Keeper) BeginBlocker(ctx sdk.Context) {
	...
	if ctx.BlockHeight()%types.ValidatorSetInterval == 0 {
		k.IterateRegisteredZones(ctx, k.syncValidatorSets(ctx))
	}
	// every N blocks, emit QueryAccountBalances event.
	if ctx.BlockHeight()%types.DepositInterval == 0 {
		k.IterateRegisteredZones(ctx, k.syncDeposits(ctx))
	}
	if ctx.BlockHeight()%types.DelegateInterval == 0 {
		k.IterateRegisteredZones(ctx, k.syncDelegateBalances((ctx)))
	}
	if ctx.BlockHeight()%types.DelegateDelegationsInterval == 0 {
		k.IterateRegisteredZones(ctx, k.syncDelegatorDelegations((ctx)))
	}
}

func (k Keeper) syncValidatorSets(ctx sdk.Context) func(int64, types.RegisteredZone) bool {
	return func(index int64, zoneInfo types.RegisteredZone) (stop bool) {
		...
		validatorData, err := k.ICQKeeper.GetDatapoint(ctx, zoneInfo.ConnectionId, zoneInfo.ChainId, "cosmos.staking.v1beta1.Query/Validators", map[string]string{"status": stakingTypes.BondStatusBonded})
		if err != nil {
			k.Logger(ctx).Error("Unable to query validators for zone", "zone", zoneInfo.ChainId)
			return false
		}
		if validatorData.LocalHeight.LT(sdk.NewInt(ctx.BlockHeight() - types.DelegateDelegationsInterval)) {
		...
		if err = k.cdc.UnmarshalJSON(validatorData.Value, &validatorsRes); err != nil {
...
func (k Keeper) syncDeposits(ctx sdk.Context) func(int64, types.RegisteredZone) bool {
	return func(index int64, zoneInfo types.RegisteredZone) (stop bool) {
		balanceData, err := k.ICQKeeper.GetDatapoint(ctx, zoneInfo.ConnectionId, zoneInfo.ChainId, "cosmos.bank.v1beta1.Query/AllBalances", map[string]string{"address": zoneInfo.DepositAddress.GetAddress()})
		if err != nil {
		...
		if err = k.cdc.UnmarshalJSON(balanceData.Value, &balanceRes); err != nil {
		...
		if balance := balanceRes.Balances; !balance.Empty() {
			k.Logger(ctx).Info("Balance is non zero", "existing", zoneInfo.DepositAddress.Balance, "current", balance)
			txData, err := k.ICQKeeper.GetDatapointOrRequest(ctx, zoneInfo.ConnectionId, zoneInfo.ChainId, "cosmos.tx.v1beta1.Query/GetTxEvents", map[string]string{"transfer.recipient": zoneInfo.DepositAddress.GetAddress()})
			if err != nil {
			...
			if err := json.Unmarshal(txData.Value, &txs); err != nil {
...
func (k Keeper) syncDelegateBalances(ctx sdk.Context) func(int64, types.RegisteredZone) bool {
	return func(index int64, zoneInfo types.RegisteredZone) (stop bool) {
		for _, da := range zoneInfo.DelegationAddresses {
			balanceData, err := k.ICQKeeper.GetDatapoint(ctx, zoneInfo.ConnectionId, zoneInfo.ChainId, "cosmos.bank.v1beta1.Query/AllBalances", map[string]string{"address": da.GetAddress()})
			if err != nil {
				k.Logger(ctx).Error("Unable to query balance for delegate account", "delegate_address", da.GetAddress())
				continue
			}
			if balanceData.LocalHeight.LT(sdk.NewInt(ctx.BlockHeight() - types.DelegateInterval)) {
			...
			if balance := balanceRes.Balances; !balance.Empty() {
				...
				if err := k.Delegate(ctx, zoneInfo, da); err != nil {
...
func (k Keeper) syncDelegatorDelegations(ctx sdk.Context) func(int64, types.RegisteredZone) bool {
	return func(index int64, zoneInfo types.RegisteredZone) (stop bool) {
		// populate / handle delegations
		for _, da := range zoneInfo.DelegationAddresses {
			delegationData, err := k.ICQKeeper.GetDatapoint(ctx, zoneInfo.ConnectionId, zoneInfo.ChainId, "cosmos.staking.v1beta1.Query/DelegatorDelegations", map[string]string{"address": da.GetAddress()})
			if err != nil {
				k.Logger(ctx).Error("Unable to query balance for delegate account", "delegate_address", da.GetAddress())
				continue
			}
			if delegationData.LocalHeight.LT(sdk.NewInt(ctx.BlockHeight() - types.DelegateDelegationsInterval)) {
			...
			if err := k.cdc.UnmarshalJSON(delegationData.Value, &delegationsRes); err != nil {
