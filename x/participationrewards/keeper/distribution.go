...
func (k *Keeper) SetZoneAllocations(ctx sdk.Context, tvs TokenValues, allocation types.RewardsAllocation) error {
	...
	k.icsKeeper.IterateZones(ctx, func(index int64, zone *icstypes.Zone) (stop bool) {
		...
		if !exists {
			k.Logger(ctx).Error(fmt.Sprintf("unable to obtain token value for zone %s", zone.ZoneID()))
			return false
		}
		...
		k.Logger(ctx).Info("zone tvl", "zone", zone.ZoneID(), "tvl", ztvl)
	...
	k.icsKeeper.IterateZones(ctx, func(index int64, zone *icstypes.Zone) (stop bool) {
		...
		k.Logger(ctx).Info("zone proportion", "zone", zone.ZoneID(), "proportion", zp)
