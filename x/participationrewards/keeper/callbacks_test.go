...
func (s *KeeperTestSuite) executeOsmosisPoolUpdateCallback() {
	prk := s.GetQuicksilverApp(s.chainA).ParticipationRewardsKeeper
...
func (s *KeeperTestSuite) executeValidatorSelectionRewardsCallback(performanceAddress string, valRewards map[string]sdk.Dec) {
	prk := s.GetQuicksilverApp(s.chainA).ParticipationRewardsKeeper
	ctx := s.chainA.GetContext()
...
func (s *KeeperTestSuite) executeSetEpochBlockCallback() {
	prk := s.GetQuicksilverApp(s.chainA).ParticipationRewardsKeeper
	ctx := s.chainA.GetContext()
	...
	err := s.GetQuicksilverApp(s.chainA).AppCodec().UnmarshalJSON([]byte(respJSON), &glbrResp)
	s.Require().NoError(err)
