...
func (suite *KeeperTestSuite) Test_Membrane_ValidateClaim() {
	...
	for _, c := range cases {
		suite.Run(c.name, func() {
			...
			err = json.Unmarshal([]byte(testProofOps), &proofOps)
			suite.NoError(err)
