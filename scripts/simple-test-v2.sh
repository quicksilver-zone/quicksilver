...
#Saving DEMO_ADDRESS_2 account balance (uatom) in response 1
response1=$($TZ1_1_EXEC query bank balances $DEMO_ADDRESS_2 --output=json | jq .balances[0].amount -r)
sleep 5
$TZ1_1_EXEC tx bank send demowallet2 $DEPOSIT_ACCOUNT ${DEL_AMT1}uatom --chain-id $CHAINID_1 -y --keyring-backend=test
sleep 5
$TZ1_1_EXEC tx bank send demowallet2 $DEPOSIT_ACCOUNT ${DEL_AMT2}uatom --chain-id $CHAINID_1 -y --keyring-backend=test --note MgTUzEjWVVYoDZBarqFL1akb38mxlgTsqdZ/sFxTJBNf+tv6rtckvn3T
sleep 5
$TZ1_1_EXEC tx bank send demowallet2 $DEPOSIT_ACCOUNT ${DEL_AMT3}uatom --chain-id $CHAINID_1 -y --keyring-backend=test
sleep 5
#Saving DEMO_ADDRESS_2 account balance (uatom) after send txs in response 2
response2=$($TZ1_1_EXEC query bank balances $DEMO_ADDRESS_2 --output=json | jq .balances[0].amount -r)
sum=$((response2 + DEL_AMT1 + DEL_AMT2 + DEL_AMT3))
if [ "$sum" -eq "$response1" ]; then
    echo "The uatom balance matches"
else
    echo "Balance differs : uatom"
fi
sleep 5
#Getting the redemption rate from quicksilver chain
redemptionRate=$($QS1_EXEC q interchainstaking zones --output=json | jq .zones[0].redemption_rate -r)
echo "redemption rate : $redemptionRate"
sleep 100
# When mapped account will be given - use this code for qtokens
#response3=$($QS1_EXEC q bank balances $DEMO_ADDRESS_22 --output=json | jq .balances[0].amount -r)
totalDel=$((DEL_AMT1+DEL_AMT2+DEL_AMT3))
qTokens=$(echo "$totalDel * $redemptionRate" | bc -l)
qTokensInt=$(printf "%.0f" "$qTokens")
#if [ "$qTokensInt" -eq "$response3" ]; then
#    echo "All Good - qtokens match according to the redemption rate"
#else
#    echo "Balance differs"
#fi
# When mapped account is not  given - use this code for qtokens
qatomBal=$($TZ1_1_EXEC query bank balances $DEMO_ADDRESS_2 --output=json | jq .balances[0].amount -r)
if [ "$qTokensInt" -eq "$qatomBal" ]; then
    echo "The qatom balance matches"
else
    echo "Balance differs : uatom"
fi
