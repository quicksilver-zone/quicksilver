...
docker-compose exec testzone icad tx staking tokenize-share $VAL_VALOPER_2 10000stake $VAL_ADDRESS_2 --keyring-backend test --from val2 --chain-id $CHAINID_2 --gas 400000 -y

sleep 10
DEPOSIT_ACCOUNT=$($QS_EXEC q interchainstaking zones --output=json | jq .zones[0].deposit_address.address -r)

sleep 10
docker-compose exec testzone icad tx bank send val2 $DEPOSIT_ACCOUNT 10000${VAL_VALOPER_2}1  --keyring-backend test --chain-id $CHAINID_2 -y

sleep 10
docker-compose exec testzone icad tx staking delegate ${VAL_VALOPER_2} 36000stake --keyring-backend test --from demowallet2 --chain-id $CHAINID_2 -y

sleep 10

docker-compose exec testzone icad tx staking tokenize-share $VAL_VALOPER_2 36000stake $VAL_ADDRESS_2 --keyring-backend test --from demowallet2 --chain-id $CHAINID_2 --gas 400000 -y

sleep 30

docker-compose exec testzone icad tx bank send demowallet2 $DEPOSIT_ACCOUNT 20000${VAL_VALOPER_2}2  --keyring-backend test --chain-id $CHAINID_2 -y

sleep 30

docker-compose exec testzone icad tx bank send demowallet2 $DEPOSIT_ACCOUNT 10000${VAL_VALOPER_2}2  --keyring-backend test --chain-id $CHAINID_2 -y

sleep 30

docker-compose exec testzone icad tx bank send demowallet2 $DEPOSIT_ACCOUNT 6000${VAL_VALOPER_2}2  --keyring-backend test --chain-id $CHAINID_2 -y
